...
submitForm(event: Event) {   
  ...
  formData.append('files', new Blob([JSON.stringify([event.target as HTMLInputElement | null][0]?.files?.item(0)?.arrayBuffer.toString())], {type:  this.showEvent?.type}), this.showEvent?.name);
  this.res = [event.target as HTMLInputElement | null][0]?.files?.item(0)?.name;
  const apiToken =
  'h2yMCGyvkuFk1yVedt6bl9ukSFP88jD6BkEHtJQatY2midcZ7MLJp3um+m9r/lYHYEz++5U6R9JSOh4L4GAuopf4DW7oAzESKhcEShhQ983JKmXg7uWghzNuW+p4wfvVqsMcrr5RkmMHu3CiZDo9PQ==';
...
const response2 = this.http.post(this.url,formData,{headers:this.headers2});
console.log(response);
